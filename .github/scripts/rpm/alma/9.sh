...
# ====================================================================================
echo "Updating..." && dnf update -y > /dev/null 2>&1
...
echo "Installing build tools..." && dnf install --allowerasing -y wget zip unzip gcc gcc-c++ make openssl-devel curl nano git jq \
    ...
    re2-devel rsync GeoIP GeoIP-devel pkg-config diffutils file luajit luajit-devel > /dev/null 2>&1
# ====================================================================================
...
cd $GITHUB_WORKSPACE/nginx_mods/boringssl-$BORINGSSL_VERSION; mkdir -p build; cd build; cmake -DCMAKE_POSITION_INDEPENDENT_CODE=ON .. > /dev/null 2>&1; echo "Building BoringSSL..." && make -j$CORES > /dev/null 2>&1
